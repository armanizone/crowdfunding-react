...
const rewardSchema = yup.object({
  title: yup.string().max(70, 'Максимальное количество символов 50').required('Название обязательно для заполнения'),
  description: yup.string().max(500, 'Максимальное количество символов 500').required('Описание обязательно для заполнения'),
  how_to_get: yup.string().max(240, 'Максимальное количество символов 240'),
  image: yup.string().nullable(),
  city: yup.string().nullable(),
  cost: yup.number().typeError('Укажите стоимость вознаграждения').min(100, 'Минимальное стоимость 100 тг').required('Стоимость обязательна для заполнения'),
  count: yup.number().typeError('Укажите количество вознаграждений').min(5, 'Минимальное количество 5').required('Количество обязательно для заполнения'),
  sending: yup.date(),
})
